...
*   **🧪 Test Generation:** Automatically compiles `gen.cpp` and generates inputs into `gen_tests/`. Flag `-g` to generate without running.
*   **🔥 Stress Testing:** Flag `-S` runs a brute-force comparison.
*   **🛡️ Isolation:** By default, tests run in `/tmp`, keeping your project clean.
*   **✨ Auto-Fix:** Flag `-F` automatically fixes formatting and linter errors.
*   **🧹 Clean Compile:** Flag `-R` **permanently removes** comments.
*   **🔍 Static Analysis:** Integrated `clang-tidy` and `clang-format` checks.
...
Use the prompt below to ask ChatGPT/Claude to write a generator for you.
...
| `-g`, `--gen-only`
| **Generate Only.** Compile `gen.cpp`, create tests in `gen_tests/`, and exit.
